...
class Drunk(object):
    def __init__(self, name):
        self.name = name
    def move(self, field, time = 1):#set up a default value for time
...
##drunk = Drunk('Kyle Feng')
##for i in range(3):
##    f = Field(drunk, Location(0,0))
##    distances = performTrial(1, f)
##    pylab.plot(distances)
##pylab.title('Nomal random walk')
##pylab.xlabel('time')
...
def ansQuest(maxTime, numTrials):
    ...
    pylab.xlabel('time')
    pylab.title('average distance of time: ' + str(len(distLists)))
